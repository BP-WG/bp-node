...
use bitcoin::{consensus::deserialize, hashes::Hash, BlockHash, Txid};
use diesel::{prelude::*, result::Error as DieselError, PgConnection};

use super::db::model::{self, *};
use super::Error;
use crate::parser::{data::*, BulkParser, State as ParserState};
...
impl ParserState {
    pub(super) fn restore(
        ...
        let block_cache = cache_table
            .load::<CachedBlock>(state_conn)
            .or::<DieselError>(Ok(Vec::new()))?
            .into_iter()
            .try_fold(
                BlockMap::new(),
                |mut map, block| -> Result<BlockMap, Error> {
                    map.insert(
                        BlockHash::from_slice(&block.hash[..])
                            .map_err(|_| Error::IndexDBIntegrityError)?,
    ...
    pub(super) fn store(
        ...
        // Updating state data
        diesel::update(state_table.find(0))
        ...
        // Saving block cache
        let values: Vec<model::CachedBlock> = self
            .block_cache
            .iter()
        ...
        // Saving UTXOs
        let values: Vec<model::Utxo> = self
            .utxo
            .iter()
            .flat_map(|(txid, vout_map)| -> Vec<model::Utxo> {
                vout_map
                    .iter()
                    .map(|(vout, descriptor)| -> model::Utxo {
                        model::Utxo {
                            txid: txid.to_vec(),
                            block_height: descriptor.get_block_height().expect(
                                "Runtime error 5: parser-generated descriptor fails conversion",
                            ) as i32,
                            block_checksum: descriptor.get_block_checksum().expect(
                                "Runtime error 6: parser-generated descriptor fails conversion",
                            ) as i16,
                            tx_index: descriptor.get_tx_index().expect(
                                "Runtime error 6: parser-generated descriptor fails conversion",
                            ) as i16,
